...
import { ChatHistoryPrompt, convertChatContextHistory } from "./history";
 ...
 */
export class FilePrompt extends PromptElement<IMyPromptProps> {
  render() {
    return (
      <>
        <ChatHistoryPrompt
          history={convertChatContextHistory(this.props.history)}
          userQuery={this.props.userQuery}
          baseInstructions="Here are your base instructions."
          mode="file_contents"
          flexGrow={2}
          flexReserve="/5"
        />
...
> {
  ...
  private async getExpandedFiles(sizing: PromptSizing) {
    const files = this.props.files.map(
      (f) => new FileContextTracker(f.document, f.line)
    );
...
class FileContextTracker {
  ...
  constructor(
    ...
    private readonly originLine: number
  ) {}
